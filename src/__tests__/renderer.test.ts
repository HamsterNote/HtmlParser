...
import type { DecodeOptions } from '../index.js'
...
describe('decodeToHtml textControl overrides', () => {
  ...
  it('does not mutate source text/page/document after decode with overrides', async () => {
    ...
    const options: DecodeOptions = {
      textControl: { color: '#e11d48', fontSize: 99, fontWeight: 900, italic: false, vertical: false }
    }
    ...
    expect(pagesAfter.map((p) => JSON.parse(JSON.stringify(p)))).toEqual(snapPages)
