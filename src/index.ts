...
type IframeHostDocument = Pick<
	Document,
	"createElement" | "body" | "documentElement"
>;
...
export const __getHtml2CanvasLoader = (): Html2CanvasLoader =>
	...
	(async () => {
		...
		return (
			(mod as unknown as { default?: Html2CanvasLike }).default ??
...
function buildInlineStyleMap(styleText: string | null): Map<string, string> {
	...
	devConsoleLog("[buildInlineStyleMap] 解析 style 属性", {
		styleText,
...
function parseInlineStyle(el: Element): ParsedInlineStyle {
	devConsoleLog("[parseInlineStyle] 解析元素行内样式", {
		tagName: el.tagName,
...
function parseTransformLength(raw: string, percentBasis = 0): number {
	if (!raw) return 0;
	if (raw.endsWith("%")) {
...
function parseTransformMatrix(
	...
	const operations = Array.from(
		transform.matchAll(/([a-zA-Z0-9]+)\(([^)]*)\)/g),
	);
	...
	return operations.reduce<Matrix2D>((matrix, [, name, argString]) => {
		...
		switch (name) {
			...
			case "translateX":
				return multiplyMatrix2D(
					...
					createTranslationMatrix(
						parseTransformLength(args[0] ?? "0", width),
						0,
					),
				);
			case "translateY":
				return multiplyMatrix2D(
					...
					createTranslationMatrix(
						0,
						parseTransformLength(args[0] ?? "0", height),
					),
				);
			case "scale": {
			...
			case "scaleX":
				return multiplyMatrix2D(
					matrix,
					createScaleMatrix(parseTransformNumber(args[0] ?? "1"), 1),
				);
			case "scaleY":
				return multiplyMatrix2D(
					matrix,
					createScaleMatrix(1, parseTransformNumber(args[0] ?? "1")),
				);
			case "rotate":
				return multiplyMatrix2D(
					matrix,
					createRotateMatrix(parseTransformAngle(args[0] ?? "0deg")),
				);
			default:
				return matrix;
		}
...
function buildSegmentPolygon(
	...
	return transformedCorners.map(
		([x, y]) => [x + baseX, y + baseY] as [number, number],
	) as [[number, number], [number, number], [number, number], [number, number]];
}

function parseComputedPixelValue(raw: string, fallback: number): number {
...
function collectWrappedLineSegments(
	...
	return lineRects
		...
		.filter(
			(segment): segment is { content: string; rect: RectLike } =>
				segment != null,
		);
}
...
export class HtmlParser extends DocumentParser {
	...
	private static collectAncestorInlineStyle(
		...
		while (cur && cur.nodeType === ELEMENT_NODE) {
			...
			devConsoleLog(`[collectAncestorInlineStyle] 第 ${depth} 层祖先`, {
				tagName: el.tagName,
				parsed,
			});
	...
	private static measureTextMetrics(
		...
		devConsoleLog("[measureTextMetrics] 开始测量文本", {
		...
		const fallback = HtmlParser.estimateTextMetrics(
			content,
			fontSize,
			lineHeight,
		);
		...
		try {
			...
			devConsoleLog("[measureTextMetrics] pretext 测量失败，使用启发式估算", {
				error: e,
				fallback,
			});
			return fallback;
	...
	private static getComputedTextStyle(
		...
		try {
			...
			const fontSize = parseComputedPixelValue(
				style.fontSize,
				defaults.fontSize,
			);
			...
			return {
				...
				writingMode:
					parseWritingModeValue(style.writingMode) ?? "horizontal-tb",
	...
	private static collectRenderedTextSegments(
		...
		while (currentNode) {
			...
			const hasMultipleRects =
				nodeRects.filter((rect) => isValidRect(rect)).length > 1;

			if (hasMultipleRects) {
				const lineSegments = collectWrappedLineSegments(
					textNode,
					raw,
					nodeRects,
				);
	...
	private static buildRenderedTexts(
		...
		const texts = sortedSegments.map((segment, index) => {
			...
			const { ascent, descent } = estimateFontMetrics(
				segment.style.fontSize,
				height,
			);
		...
		const maxRight = texts.reduce(
			(max, text) => Math.max(max, text.polygon[1][0]),
			0,
		);
		const maxBottom = texts.reduce(
	...
	private static async withIframeDocument<T>(
		...
		try {
			...
			if ("srcdoc" in iframe) {
				...
				if (
					srcdocDocument.body.childNodes.length === 0 &&
					html.trim().length > 0
				) {
					srcdocDocument.open();
				...
				return await callback({
					iframe,
					iframeDocument: srcdocDocument,
					iframeWindow: srcdocWindow,
				});
			} else {
	...
	private static collectTextsFromDocumentFallback(
		...
		const flushPendingLine = () => {
			...
			devConsoleLog("[flushPendingLine] 刷新行", {
				segmentCount: pendingLine.length,
				currentY: y,
			});
		...
		devConsoleLog("[collectTextsFromDocument] 解析完成", {
	...
	private static fallbackPlainText(
		...
		devConsoleLog("[fallbackPlainText] 进入纯文本回退模式", {
		...
		devConsoleLog("[fallbackPlainText] 回退处理完成", {
			textCount: texts.length,
			pageHeight,
		});
	...
	static async encode(fileOrBuffer: ParserInput): Promise<HtmlDocument> {
		...
		devConsoleLog("[encode] 转换为 ArrayBuffer", {
	...
	static async decodeToHtml(
		...
		devConsoleLog("[decodeToHtml] 开始渲染 HTML 片段", {
	...
	static async decode(
		...
		devConsoleLog("[decode] 开始生成完整 HTML 文件", {
			docId: intermediateDocument.id,
		});
		...
		try {
			...
			devConsoleLog("[decode] 生成 File 对象成功", {
				name: file.name,
				size: file.size,
			});
			return file;
